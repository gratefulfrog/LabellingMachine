...
public class labellingMachine_4 extends PApplet {
...
class App{
  ...
  App(Config c, BlockingMgr b, SimuMgr smm, CommsMgr cm){
    ...
    sM = smm;
    cMgr = cm;
    tagger   = new Driver(1, config, bM, sM, sM.platform.tagRamp);
    labeller = new Driver(2, config, bM, sM, sM.platform.labelRamp);
    backer   = new Driver(3, config, bM, sM, sM.platform.base);
     if (isSimulation){
...
class Driver{
  ...
  */
  ...
  public boolean eltAtPoint(ArrayList <Sticker> al, int pt){
  ...
  public boolean eltBetweenPoints(ArrayList <Sticker> al, int lowPoint, int highPoint){
    for (int i=0;i<al.size();i++){
...
class Platform {
  ...
  Platform(Config  c){
    conf = c;
    base      =  new SteppingLine(conf.baseLength,conf.platformColor,0, conf.steps2Pixels);
    tagRamp   =  new SteppingLine(conf.rampSlopeLength,conf.platformColor,0,conf.steps2Pixels);
    labelRamp =  new SteppingLine(conf.rampSlopeLength,conf.platformColor,0,conf.steps2Pixels);
  }
  
  public void drawRamp(SteppingLine l,int lineWeight){
    strokeWeight(lineWeight);
    stroke(conf.platformColor);
    pushMatrix();
    translate(conf.rampBaseLength,0);
    rotate(conf.RAdegrees*PI/180.0f );
  ...
  public void drawBase(SteppingLine l){
    l.draw();
    //stroke(conf.platformColor);
...
class SteppingLine{
  ...
  SteppingLine(float l, int c, int bc, float stp){
    length = l;
    col = c;
    bg = bc;
    steps = 0;
...
class Sticker extends Sticker_{
 float startX,
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "labellingMachine_4" };
    if (passedArgs != null) {
